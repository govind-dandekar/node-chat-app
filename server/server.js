...
var users = new Users(); //enables us to call all user methods
...
io.on('connection', (socket) => {
  ...
  socket.on('join', (params, callback)=>{
    if (!isRealString(params.name) || !isRealString(params.room)){
      ...
      //io.emit -- emit to every connected users
      //socket.broadcast.emit -- send to every connected expect for the current user
      //socket.emit -- emit even to one user

      //emit event to room: io.to(params.room).emit();
      //socket.broadcast.to(params.room).emit()
      //socket.emit -- same w/ || w/o room
